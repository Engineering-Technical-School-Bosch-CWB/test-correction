...
arUcosCornerPositions = { arUcoPoints[0]: (0, 0), arUcoPoints[1]: (0, 0), arUcoPoints[2]: (0, 0), arUcoPoints[3]: (0, 0) }
...
def gen():
    ...
    while(vid_capture.isOpened()):
        ...
        if ret == True:
            ...
            imgWarped = utils.warp(arUcosCornerPositions, frame, imgWidth, imgHeight, False)
            ...
            imgQuestions = imgWarped[upLeftCirclesRecY:downRightCirclesRecY, upLeftCirclesRecX:downRightCirclesRecX]
            ...
            boxes, considerQuestion = utils.splitBoxes(imgTresh, questions, options, questionsOverlapMargin)
            ...
            questionValues, grade, answers = utils.giveGrades(questionValues, correctAnswers)
            global answersList
            answersList = answers
            blankImage = cv2.resize(cv2.cvtColor(np.zeros_like(imgQuestions), cv2.COLOR_GRAY2BGR), (640,640))
            blankImageWithFeedback = utils.showAnswers(blankImage, questionValues, questions, options)
            blankImageWithFeedback = cv2.resize(blankImageWithFeedback, (imgQuestionsW, imgQuestionsH))

            resizedImageWithFeedback = np.zeros_like(imgWarped)
            resizedImageWithFeedback[upLeftCirclesRecY:downRightCirclesRecY, upLeftCirclesRecX:downRightCirclesRecX] = blankImageWithFeedback
            imgFinal = cv2.addWeighted(imgWarped, 1, resizedImageWithFeedback, 1, 0)
